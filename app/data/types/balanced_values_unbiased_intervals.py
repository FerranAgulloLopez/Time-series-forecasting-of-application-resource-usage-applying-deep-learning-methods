...
class DataTypeBalancedValuesUnbiasedIntervals(DataTypeAbstract):
    ...
    def create_data_loader(
        ...
        if is_train:
            ...
            for time_series_values_index in range(len(time_series_values_values_list)):
                time_series_values = time_series_values_values_list[time_series_values_index][
                    target_feature_index]  # only target feature
                ...
                for time_position in range(max(self.window_size, self.lag_size),
                                           time_series_values.shape[0] - self.prediction_size + 1):
            ...
            for time_series_index in range(len(time_series_initial_values_list)):
                ...
                for time_position in range(1, final_predictions[time_series_index].shape[0]):
                    if final_predictions[time_series_index][time_position] == np.Infinity:
                        final_predictions[time_series_index][time_position] = final_predictions[time_series_index][
                            time_position - 1]
